...
class LSOMD(OMDBase):
  def __init__(
      ...
      self.cur_loss_hat = np.zeros_like(self.current_policy.action_probability_array).reshape((-1, 1))
  ...
  def compute_loss_hat(self, layered_plan_list, composite_feature, trajectory, step = None):
    loss_hat = np.zeros_like(self.cur_loss_hat)
    ...
    for h in range(self.H):
      ...
      if self.use_cum_statistics:
        ...
        self.cur_loss_hat[XA_idx_start: XA_idx_end] += loss_hat_use_cum
      else:
        self.cur_loss_hat[XA_idx_start: XA_idx_end] += loss_hat[XA_idx_start: XA_idx_end]
    if step is not None and step % self.degbug_length == 0:
      ic(step, loss_hat[XA_idx_start: XA_idx_end].reshape(-1,self.A), \
        ...
        self.cur_loss_hat.reshape(-1,self.A), self.current_policy.action_probability_array, layered_plan_list[-1])
  ...
  def update(self, step, trajectory):
    ...
    tem_J = np.zeros_like(self.uniform_policy)
    tem_cur_loss_hat = self.cur_loss_hat.reshape((-1, self.A))
    for h in range(self.H-1, -1, -1):
      ...
      tem_J[self._infoset_idx(h):self._infoset_idx(h+1),:] = -self.base_learning_rate*tem_cur_loss_hat[self._infoset_idx(h):self._infoset_idx(h+1),:]\
        +np.log(tem_Z[self._infoset_idx(h+1):self._infoset_idx(h+2)].reshape((-1,self.A)))
      ...
      tem_Z[self._infoset_idx(h):self._infoset_idx(h+1)]=\
        np.sum(np.multiply(self.current_policy.action_probability_array[self._infoset_idx(h):self._infoset_idx(h+1),:], _tem_Z2), axis=1)
      ...
      self.current_policy.action_probability_array[self._infoset_idx(h):self._infoset_idx(h+1),:]=\
        np.multiply(self.current_policy.action_probability_array[self._infoset_idx(h):self._infoset_idx(h+1),:],_tem_Z2)
